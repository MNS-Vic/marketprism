...
- IP 白名单（两机全端口互通 + 容器必要端口）
  - 新增 FULLMESH_IPS：两台机器之间“全端口互通”，在 scripts/manage.conf 配置。
  - 当前默认：`FULLMESH_IPS="43.156.224.10,43.156.224.10"`（你日后将第二个换成远端公网IP）。
  - 生效方式（需 sudo）：
    ```bash
    # 修改脚本配置后应用：
    vim scripts/manage.conf  # 改 FULLMESH_IPS="43.156.224.10,<远端公网IP>"
    cd services/cold-storage-service
    bash scripts/manage.sh whitelist:apply
    ```
  - 保留容器网段必要端口放行（172.16.0.0/12 → 8085/8086/8087/8123/8124/4222/8222/9000/9001/9092/9094/9095），不影响监控与数据链路。

- NATS 监控（最健康方案，已生效）
  - nats-exporter 直接加入 `message-broker_default` 网络，目标 `http://nats:8222`，避免容器→宿主发夹/防火墙链路问题。
  - Prometheus 抓取目标：`nats-exporter:7777`（完整 varz/connz/subz/jsz 指标）。
  - 热重载配置：
    ```bash
    curl -X POST http://43.156.224.10:9090/-/reload
    ```
  - 验证目标：
    ```bash
    curl -s http://43.156.224.10:9090/api/v1/targets | jq -r '.data.activeTargets[] | select(.labels.job=="nats") | [.labels.instance, .health] | @tsv'
    # 预期：nats-exporter:7777  up
    ```

- 告警规则修正
  - 将黑盒探针告警中 `instance` 从 `http://host.docker.internal:...` 修正为 `http://43.156.224.10:...`，避免“永不触发”。

- 临时文件清理（已完成）
  - 删除：temp/run_verify_now.sh、temp/run_verification.sh、temp/verify_latest.txt
  - 保留：temp/verify_20251029_144404.log（验证留档）

- 注意事项
  - FULLMESH_IPS 仅控制“本机对指定来源IP的入站全端口放行”。若需“双向互通”，请在对端也配置等价规则后各自应用。
  - 若 NATS 不在本机：建议将 nats-exporter 部署到 NATS 同主机，并让 Prometheus 直接抓取该 exporter；或在同一 overlay/network 内直连 `nats:8222`。
