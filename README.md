...
使用统一启动脚本快速启动MarketPrism：

```bash
# 开发环境启动
./start-marketprism.sh dev

# 生产环境启动
./start-marketprism.sh prod

# 仅启动核心服务
./start-marketprism.sh --core-only

# 检查环境配置
./start-marketprism.sh --check

# 停止所有服务
./start-marketprism.sh --stop
```
