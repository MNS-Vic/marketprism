...
WRITER_TYPES = {
    ...
    'optimized_clickhouse': OptimizedClickHouseWriter,
    'clickhouse_writer': ClickHouseWriter,  # 别名
    'optimized': OptimizedClickHouseWriter,  # 别名
}


def create_clickhouse_writer(config: Optional[Dict[str, Any]] = None) -> ClickHouseWriter:
...
def create_optimized_writer(config: Optional[Dict[str, Any]] = None) -> OptimizedClickHouseWriter:
    ...
    Returns:
        OptimizedClickHouseWriter实例
    """
    logger.info("创建优化版ClickHouse写入器")
    return OptimizedClickHouseWriter(config)


def get_writer_instance(writer_type: str, config: Optional[Dict[str, Any]] = None) -> Union[ClickHouseWriter, OptimizedClickHouseWriter]:
    """TDD改进：根据类型获取writer实例
    
    Args:
        writer_type: writer类型 ('clickhouse', 'optimized_clickhouse')
        config: 配置字典
...
def create_writer_from_config(config: Dict[str, Any]) -> Union[ClickHouseWriter, OptimizedClickHouseWriter]:
    """TDD改进：从配置创建writer
...
# 向后兼容的工厂函数
def create_storage_writer(config: Optional[Dict[str, Any]] = None) -> ClickHouseWriter:
    """向后兼容：创建存储写入器"""
    return create_clickhouse_writer(config)


def create_default_writer(config: Optional[Dict[str, Any]] = None) -> ClickHouseWriter:
    """创建默认writer"""
    return create_clickhouse_writer(config) 