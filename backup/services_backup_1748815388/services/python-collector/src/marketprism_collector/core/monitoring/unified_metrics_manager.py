...
class UnifiedMetricsManager:
    ...
    def add_alert_rule(self, rule: AlertRule) -> None:
        """添加告警规则"""
        self.alert_rules[f"{rule.metric_name}_{len(self.alert_rules)}"] = rule
        logger.info(f"添加告警规则: {rule.metric_name} {rule.condition} {rule.threshold}")
    ...
    def check_alerts(self) -> List[Dict[str, Any]]:
        ...
        for rule_id, rule in self.alert_rules.items():
            ...
            try:
                ...
                for label_key, metric_value in metric.get_values().items():
                    if self._matches_labels(metric_value.labels, rule.labels_filter):
                        if self._evaluate_condition(metric_value.value, rule.condition, rule.threshold):
                            alert_key = f"{rule_id}_{label_key}"
                            now = datetime.utcnow()
                            ...
                            if alert_key in self.active_alerts:
                                ...
                                if duration >= rule.duration:
                                    ...
                                    # 增加告警计数
                                    self.increment("metrics_manager_alerts_total", 1, {
                                        "severity": rule.severity.value,
                                        "metric_name": rule.metric_name
                                    })
                ...
                logger.error(f"检查告警规则失败 {rule_id}: {e}")
