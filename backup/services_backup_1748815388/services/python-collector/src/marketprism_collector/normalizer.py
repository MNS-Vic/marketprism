...
class DataNormalizer:
    ...
    def normalize_okx_ticker(self, raw_data: dict, symbol: str) -> Optional[NormalizedTicker]:
        ...
        try:
            ...
            return NormalizedTicker(
                ...
                open_time=datetime.utcnow(),  # OKX没有提供
                close_time=datetime.utcnow(),  # OKX没有提供
    ...
    def normalize_binance_trade(self, raw_data: dict) -> Optional[NormalizedTrade]:
        ...
        try:
            ...
            self.logger.error("标准化Binance交易数据失败", error=str(e), raw_data=raw_data)
            return None
    ...
    def normalize_binance_orderbook(self, raw_data: dict, symbol: str) -> Optional[NormalizedOrderBook]:
        ...
        try:
            ...
            return NormalizedOrderBook(
                ...
                bids=bids,
                asks=asks,
                timestamp=datetime.utcnow(),  # Binance depth没有时间戳
                last_update_id=raw_data.get("lastUpdateId")
            )
        except Exception as e:
            self.logger.error("标准化Binance订单簿数据失败", error=str(e), raw_data=raw_data)
            return None
    ...
    def normalize_binance_ticker(self, raw_data: dict) -> Optional[NormalizedTicker]:
        ...
        try:
            return NormalizedTicker(
                ...
                open_time=datetime.fromtimestamp(raw_data.get("O", 0) / 1000) if raw_data.get("O") else datetime.utcnow(),
                close_time=datetime.fromtimestamp(raw_data.get("C", 0) / 1000) if raw_data.get("C") else datetime.utcnow(),
                trade_count=raw_data.get("n", 0),
                timestamp=datetime.fromtimestamp(raw_data["E"] / 1000)
            )
        except Exception as e:
            self.logger.error("标准化Binance行情数据失败", error=str(e), raw_data=raw_data)
            return None
