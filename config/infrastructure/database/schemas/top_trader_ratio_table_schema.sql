-- MarketPrism 大户多空持仓比数据表结构
-- 用于存储Binance和OKX的大户/精英交易员多空持仓比数据
...
(
    ...
    -- 核心持仓比数据
    long_short_ratio Decimal64(8) COMMENT '多空持仓量比值',
    long_position_ratio Decimal64(8) COMMENT '多仓持仓量比例 (0-1)',
    short_position_ratio Decimal64(8) COMMENT '空仓持仓量比例 (0-1)',
    
    -- 账户数据 (可选，主要来自其他API)
    long_account_ratio Nullable(Decimal64(8)) COMMENT '多仓账户比例 (0-1)',
    short_account_ratio Nullable(Decimal64(8)) COMMENT '空仓账户比例 (0-1)',
    long_short_account_ratio Nullable(Decimal64(8)) COMMENT '多空账户数比值',
    
    -- 元数据
    data_type LowCardinality(String) COMMENT '数据类型: position/account',
    ...
    ratio_sum_check Nullable(UInt8) COMMENT '多空比例和检查 (1=通过, 0=失败)',
...
COMMENT '大户多空持仓比数据表 - 存储大户/精英交易员持仓比例历史数据';

-- 创建索引以优化查询性能
-- 按交易所和币种查询的索引
CREATE INDEX IF NOT EXISTS idx_exchange_currency ON top_trader_long_short_ratio (exchange_name, currency) TYPE minmax GRANULARITY 1;

-- 按数据周期查询的索引
CREATE INDEX IF NOT EXISTS idx_period ON top_trader_long_short_ratio (period) TYPE set(10) GRANULARITY 1;

-- 按多空比值范围查询的索引 (用于异常比值监控)
CREATE INDEX IF NOT EXISTS idx_long_short_ratio ON top_trader_long_short_ratio (long_short_ratio) TYPE minmax GRANULARITY 1;

-- 按数据质量查询的索引
CREATE INDEX IF NOT EXISTS idx_data_quality ON top_trader_long_short_ratio (data_quality_score) TYPE minmax GRANULARITY 1;

-- 创建物化视图：按小时聚合的持仓比统计
CREATE MATERIALIZED VIEW IF NOT EXISTS top_trader_ratio_hourly_stats
(
    ...
    avg_long_position_ratio Decimal64(8),
    avg_short_position_ratio Decimal64(8),
...
AS SELECT
    ...
    avg(long_position_ratio) as avg_long_position_ratio,
    avg(short_position_ratio) as avg_short_position_ratio,
    avg(data_quality_score) as avg_data_quality_score,
    count() as data_points,
    min(timestamp) as first_timestamp,
    max(timestamp) as last_timestamp
FROM top_trader_long_short_ratio
GROUP BY exchange_name, currency, period, hour;

-- 创建物化视图：按日聚合的持仓比统计
CREATE MATERIALIZED VIEW IF NOT EXISTS top_trader_ratio_daily_stats
(
    exchange_name LowCardinality(String),
    currency LowCardinality(String),
    period LowCardinality(String),
...
-- 创建用于监控的视图
-- 最新大户持仓比数据视图
CREATE VIEW IF NOT EXISTS latest_top_trader_ratio AS
SELECT 
    exchange_name,
    currency,
    symbol_name,
...
-- 大户持仓比排行榜视图 (按多空比值排序)
CREATE VIEW IF NOT EXISTS top_trader_ratio_rankings AS
SELECT 
    exchange_name,
    currency,
    symbol_name,
...
-- 大户持仓比异常监控视图 (极端比值情况)
CREATE VIEW IF NOT EXISTS top_trader_ratio_anomalies AS
SELECT 
    exchange_name,
    currency,
    symbol_name,
    ...
    CASE
        ...
        ELSE 'NORMAL'
    END as data_issue
FROM latest_top_trader_ratio
WHERE long_short_ratio > 2.0 OR long_short_ratio < 0.5 OR data_quality_score < 0.7
ORDER BY abs(long_short_ratio - 1.0) DESC;

-- 验证表创建
SELECT 
    'top_trader_long_short_ratio' as table_name,
    count() as row_count,
    formatReadableSize(sum(data_compressed_bytes)) as compressed_size,
    formatReadableSize(sum(data_uncompressed_bytes)) as uncompressed_size
FROM system.parts 
WHERE database = 'marketprism' AND table = 'top_trader_long_short_ratio'
UNION ALL
SELECT 
    'top_trader_ratio_hourly_stats' as table_name,
    count() as row_count,
    formatReadableSize(sum(data_compressed_bytes)) as compressed_size,
    formatReadableSize(sum(data_uncompressed_bytes)) as uncompressed_size
FROM system.parts 
WHERE database = 'marketprism' AND table = 'top_trader_ratio_hourly_stats'
UNION ALL
SELECT 
    'top_trader_ratio_daily_stats' as table_name,
    count() as row_count,
    formatReadableSize(sum(data_compressed_bytes)) as compressed_size,
    formatReadableSize(sum(data_uncompressed_bytes)) as uncompressed_size
FROM system.parts 
WHERE database = 'marketprism' AND table = 'top_trader_ratio_daily_stats';

-- 显示创建的表和视图
SHOW TABLES FROM marketprism LIKE '%top_trader%';

-- 显示表结构
DESCRIBE TABLE top_trader_long_short_ratio;

-- 完成提示
SELECT 'MarketPrism大户多空持仓比数据表初始化完成！' as status;
