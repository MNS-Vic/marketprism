"""
顶级大户多空持仓比例数据管理器工厂（按持仓量计算）

使用工厂模式创建不同交易所的顶级大户多空持仓比例管理器实例
"""
...
class LSRTopPositionManagerFactory:
    """
    顶级大户多空持仓比例数据管理器工厂类（按持仓量计算）

    负责根据配置创建相应的管理器实例
    专门处理顶级大户按持仓量计算的多空比例数据
    """
    ...
    def __init__(self):
        ...
        self._manager_registry = {
            (Exchange.OKX_DERIVATIVES, MarketType.DERIVATIVES): OKXDerivativesLSRTopPositionManager,
            (Exchange.BINANCE_DERIVATIVES, MarketType.DERIVATIVES): BinanceDerivativesLSRTopPositionManager,
        }
    ...
    def create_manager(self,
                      ...
                      nats_publisher: Any,
                      config: Dict[str, Any]) -> Optional[BaseLSRTopPositionManager]:
        """
        创建顶级大户多空持仓比例数据管理器
    ...
    def get_manager_info(self) -> Dict[str, Any]:
        ...
        """
        return {
            'factory_type': 'LSRTopPositionManagerFactory',
            'data_type': 'lsr_top_position',
            'supported_combinations': [
                {
                    'exchange': k[0].value,
