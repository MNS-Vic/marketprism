        ...
        try:
            ...
            if action == 'snapshot':
                ...
                if prev_seq_id != state.last_update_id:
                    return False, f"序列不连续: 期望prevSeqId={state.last_update_id}, 实际={prev_seq_id}"
    ...
    def _process_buffered_messages(self, symbol: str, state: OrderBookState) -> List[dict]:
        ...
        while buffer:
            ...
            if prev_seq_id == state.last_update_id:
                ...
                state.last_update_id = message.get('seqId')
                buffer.pop(0)
    ...
    async def _apply_update(self, symbol: str, update_data: dict, state: OrderBookState):
        ...
        try:
            ...
            if self.checksum_validation_enabled:
                ...
                if calculated_checksum != expected_checksum:
                    self.logger.warning(f"⚠️ OKX现货更新checksum验证失败: {symbol}，回滚更新")
