...
class OKXDerivativesSnapManager(BaseOrderBookSnapManager):
    """
    OKX Perpetual REST 快照管理器（GET /api/v5/market/books）。

    要点：
    - 每个 1s tick 内对每个符号发起 REST GET 请求
    - 超时或错误时跳过本 tick，不降频
    - 使用 aiohttp 会话池复用连接
    - 与 OKXSpotSnapManager 共享相同的 REST 端点，仅 instId 不同（永续为 BTC-USDT-SWAP）
    """
