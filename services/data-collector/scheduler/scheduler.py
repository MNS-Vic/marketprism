...
class CollectorScheduler:
    ...
    async def add_job(
        ...
        **trigger_kwargs
        ...
        try:
            ...
            self.logger.error("添加定时任务失败", job_id=job_id, exc_info=True)
            raise
    ...
    async def remove_job(self, job_id: str):
        ...
        try:
            ...
            self.logger.error("移除定时任务失败", job_id=job_id, exc_info=True)
            ...
            for adapter_key, adapter in self.collector.exchange_adapters.items():
                if hasattr(adapter, 'collect_funding_rate_data'):
                    try:
                        ...
                        self.logger.error("资金费率收集失败", adapter=adapter_key, exc_info=True)
            
            # 更新任务运行记录
            if "funding_rate_collection" in self._jobs:
                self._jobs["funding_rate_collection"]["last_run"] = datetime.datetime.now(datetime.timezone.utc)
                self._jobs["funding_rate_collection"]["run_count"] += 1
                
        except Exception as e:
            self.logger.error("资金费率收集任务执行失败", exc_info=True)
            ...
            for adapter_key, adapter in self.collector.exchange_adapters.items():
                if hasattr(adapter, 'collect_open_interest_data'):
                    try:
                        ...
                        self.logger.error("持仓量收集失败", adapter=adapter_key, exc_info=True)
            
            # 更新任务运行记录
            if "open_interest_collection" in self._jobs:
                self._jobs["open_interest_collection"]["last_run"] = datetime.datetime.now(datetime.timezone.utc)
                self._jobs["open_interest_collection"]["run_count"] += 1
                
        except Exception as e:
            self.logger.error("持仓量收集任务执行失败", exc_info=True)
            ...
            for adapter_key, adapter in self.collector.exchange_adapters.items():
                if hasattr(adapter, 'check_liquidation_stream'):
                    try:
                        ...
                        self.logger.error("强平监控检查失败", adapter=adapter_key, exc_info=True)
            
            # 更新任务运行记录
            if "liquidation_monitoring" in self._jobs:
                self._jobs["liquidation_monitoring"]["last_run"] = datetime.datetime.now(datetime.timezone.utc)
                self._jobs["liquidation_monitoring"]["run_count"] += 1
                
        except Exception as e:
            self.logger.error("强平监控任务执行失败", exc_info=True)
    ...
    async def _health_check(self):
        ...
        try:
                ...
                if hasattr(adapter, 'is_connected'):
                    ...
                    if not is_connected:
                        ...
                        try:
                            ...
                            self.logger.error("重连失败", adapter=adapter_key, exc_info=True)
            ...
            if "health_check" in self._jobs:
                self._jobs["health_check"]["last_run"] = datetime.datetime.now(datetime.timezone.utc)
                self._jobs["health_check"]["run_count"] += 1
                
        except Exception as e:
            self.logger.error("健康检查任务执行失败", exc_info=True)
