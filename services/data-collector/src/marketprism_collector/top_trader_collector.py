...
from .data_types import (
    Exchange, DataType, NormalizedTopTraderLongShortRatio
)


class TopTraderDataCollector:
    """大户持仓比数据收集器"""
    ...
    def __init__(self, rest_client_manager: RestClientManager):
        ...
        # 回调函数
        self.callbacks: List[Callable[[NormalizedTopTraderLongShortRatio], None]] = []
        
        # 配置
        self.symbols = ["BTC-USDT", "ETH-USDT", "BNB-USDT"]  # 默认监控的交易对
    ...
    async def _collection_loop(self):
        ...
        while self.is_running:
            try:
                ...
                self.logger.error("数据收集循环错误", exc_info=True)
    ...
    async def _collect_all_data(self):
        ...
        try:
                ...
                if isinstance(result, Exception):
                    ...
                    for callback in self.callbacks:
                        try:
                            ...
                            self.logger.error("回调函数执行失败", exc_info=True)
            ...
            self.logger.error("数据收集失败", exc_info=True)
    ...
    async def _collect_binance_data(
        ...
        try:
            ...
            # 标准化数据
            normalized_data = NormalizedTopTraderLongShortRatio(
            ...
            self.logger.error("收集Binance数据失败", symbol=symbol, exc_info=True)
            return None
    ...
    async def _collect_okx_data(
        ...
        try:
            ...
            # OKX返回格式：[timestamp, longShortPosRatio]
            timestamp_ms = int(data_point[0])
            long_short_ratio = Decimal(str(data_point[1]))
            
            # 计算多空仓位比例（假设总和为1）
            # 如果多空比为1.5，则多仓比例约为0.6，空仓比例约为0.4
            total_ratio = long_short_ratio + Decimal('1')
            long_position_ratio = long_short_ratio / total_ratio
            short_position_ratio = Decimal('1') / total_ratio
            ...
            normalized_data = NormalizedTopTraderLongShortRatio(
                ...
                raw_data={'timestamp': timestamp_ms, 'longShortPosRatio': str(long_short_ratio)}
            )
            ...
            self.logger.error("收集OKX数据失败", symbol=symbol, exc_info=True)
            return None
    
    def register_callback(self, callback: Callable[[NormalizedTopTraderLongShortRatio], None]):
        """注册数据回调函数"""
    ...
    async def collect_once(self) -> List[NormalizedTopTraderLongShortRatio]:
        """手动触发一次数据收集（用于测试）"""
        results = []
