    ...
    async def monitor_queue_sizes(adapters: dict, interval: float = 30.0):
        ...
        while True:
            try:
                ...
                logger.error("队列监控错误", error=str(e))
                await asyncio.sleep(interval)
    ...
    async def update_system_metrics(interval: float = 60.0):
        ...
        while True:
            try:
                ...
                logger.error("系统指标收集错误", error=str(e))
                await asyncio.sleep(interval)
...
class MarketDataCollector:
    ...
    def export_historical_data(self, export_params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 模拟数据量计算
            date_range_days = (datetime.fromisoformat(final_params['end_date'].replace('Z', '')) - 
                             datetime.fromisoformat(final_params['start_date'].replace('Z', ''))).days
            ...
            export_result['progress']['estimated_completion_time'] = (
                datetime.utcnow() + timedelta(hours=processing_time_hours)
            ).isoformat()
    ...
    def manage_data_retention(self, retention_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            retention_result = {
                'retention_id': f'retention_{int(datetime.utcnow().timestamp())}',
                'configuration': final_config,
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            if CORE_MONITORING_AVAILABLE and ClickHouseWriter:
                ...
                # 使用Core服务的ClickHouse写入器
                self.clickhouse_writer = core_services.get_clickhouse_writer(self.config.__dict__)
            ...
            log_collector_error("启动收集器失败", error=str(e), error_id=error_id)
            await self.stop()
            return False
    ...
    async def _start_exchange_adapters(self):
        ...
        for exchange_config in enabled_exchanges:
            try:
                # 使用工厂创建适配器
                adapter = ExchangeFactory.create_adapter(exchange_config)
    ...
    async def handle_dynamic_subscription_command(self, command_message: dict) -> dict:
        ...
        try:
            ...
            self.logger.error(error_message, error=str(e), command_id=command_id)
    ...
    async def _integrate_with_websocket_adapter(self, exchange: str, symbol: str, action: str, data_types: list) -> dict:
        """
        与WebSocket适配器集成 - TDD Phase 4 智能集成功能
        尝试将动态订阅命令应用到实际的交易所WebSocket连接
        自动检测和使用增强适配器，支持交易所特定的特性
        ...
        try:
            ...
            from .types import Exchange, ExchangeConfig, MarketType
