...
class OrderBookNATSPublisher:
    ...
    async def start(self, symbols: List[str] = None) -> bool:
        ...
        try:
            ...
            self.logger.error("启动NATS推送器失败", error=str(e))
            self.is_running = False
            return False
    ...
    async def _publish_loop(self):
        ...
        while self.is_running:
            try:
                ...
                self.logger.error("推送循环异常", error=str(e))
                self.stats['errors'] += 1
    ...
    async def _publish_orderbook(self, symbol: str) -> bool:
        ...
        try:
            ...
            if success:
                # 更新推送记录
                self.last_publish_times[symbol] = datetime.utcnow()
    ...
    def _create_publish_data(self, orderbook: EnhancedOrderBook) -> 'NormalizedOrderBook':
        """创建推送数据"""
        from .types import NormalizedOrderBook
