...
class PerformanceMonitor:
    ...
    def record_message(self, exchange: str = "unknown"):
        """记录消息处理"""
        self.message_count += 1
        if exchange not in self.exchange_message_counts:
            self.exchange_message_counts[exchange] = 0
        self.exchange_message_counts[exchange] += 1
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'exchange_message_counts': self.exchange_message_counts,
            'cpu_usage': {
...
async def test_multi_exchange_performance_final():
    """多交易所并发性能测试 - 最终版本"""
    print("🚀 多交易所并发性能测试 - 最终版本")
    print("=" * 80)
    ...
    if rest_results.get("Binance", False):
        binance_config = ExchangeConfig(
            ...
            ws_url="wss://data-stream.binance.vision/ws",  # 使用可用的备用域名
    ...
    # 暂时跳过Deribit，因为WebSocket连接仍有问题
    print("⚠️ Deribit暂时跳过 (WebSocket连接问题)")
    
    if not configs:
        print("❌ 没有可用的交易所配置，测试终止")
        return
    
    print(f"\n📊 开始测试 {len(configs)} 个交易所...")
    ...
    for config in configs:
        try:
            ...
            def create_callback(exchange_name):
                def callback(data):
                    monitor.record_message(exchange_name)
                    if monitor.message_count % 50 == 0:  # 每50条消息记录一次性能
    ...
    # 各交易所消息统计
    print(f"\n📈 各交易所消息统计:")
    for exchange, count in stats['exchange_message_counts'].items():
        percentage = count / stats['messages_processed'] * 100 if stats['messages_processed'] > 0 else 0
    ...
    # 改进建议
    print(f"\n💡 改进建议:")
    if len(stats['connection_status']) < 3:
        print(f"   🔧 添加更多交易所支持 (当前: {len(stats['connection_status'])}/3)")
    if stability < 100:
        print(f"   🔧 提高连接稳定性")
    if msg_per_sec < 50:
        print(f"   🔧 优化消息处理速度")
    ...
    # 保存详细结果
    result_file = f"multi_exchange_performance_final_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
async def main():
    """主函数"""
    try:
        await test_multi_exchange_performance_final()
