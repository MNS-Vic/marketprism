...
class TestNormalizedTrade:
    ...
    def test_trade_serialization(self):
        ...
        # 测试模型序列化 - Pydantic v1保留datetime对象
        json_data = trade.dict()
        assert json_data["exchange_name"] == "binance"
        assert str(json_data["price"]) == "50000.50"
        assert json_data["timestamp"] == timestamp  # v1的dict()保留datetime对象
...
class TestNormalizedOrderBook:
    ...
    def test_empty_orderbook(self):
        ...
        assert isinstance(json_data["timestamp"], datetime)  # Pydantic v1的dict()保留datetime对象
