"""
test_types.py - 真实组件版本（无Mock）
完全移除Mock，使用真实MarketPrism组件进行测试
"""
...
class TestNormalizedTrade:
    ...
    def test_trade_serialization(self):
        ...
        assert str(json_data["price"]) == "50000.50"
        assert isinstance(json_data["timestamp"], (str, datetime)) # Accept both formats
...
class TestNormalizedOpenInterest:
    ...
    def test_valid_open_interest(self):
        ...
        assert open_interest.open_interest_value == Decimal("10000")
        
        # 测试序列化
        json_data = open_interest.dict()
        assert str(json_data["open_interest_value"]) == "10000"
    ...
    def test_open_interest_with_changes(self):
        ...
        assert open_interest.open_interest_value == Decimal("50000")
        if hasattr(open_interest, 'sum_open_interest_value'):
            assert open_interest.sum_open_interest_value == Decimal("75000")
        
        # 测试序列化
        json_data = open_interest.dict()
        assert str(json_data["open_interest_value"]) == "50000"
...
class TestNormalizedLiquidation:
    ...
    def test_valid_liquidation(self):
        ...
        assert str(json_data["price"]) == "45000.00"
    ...
    def test_liquidation_with_optional_fields(self):
        ...
        json_data = liquidation.dict()
        assert str(json_data["value"]) == "450000.00"
        assert str(json_data["leverage"]) == "100"
